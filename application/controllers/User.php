...
class User extends CI_Controller
{
     ...
     {
          $id = ['id_peserta' => $this->session->id];
          $data['ar'] = $this->m_user->cekdefpass($id)->row_array();
          $data['title'] = 'Ujian Berbasis Komputer';
          $data['judul'] = 'Dashboard';
          $whrjurusan = ['jurusan.nama_jurusan' => $this->session->userdata('jurusan')];
     ...
     public function ujian($id_ujian)
     {
          ...
          $whr_cek_sdh_selesai = ['id_peserta' => $data['sess_id'], 'id_ujian' => $id_ujian, 'status' => 'N'];
          ...
          if ($cek_sdh_selesai < 1) {
               ...
               $acakan = $cek_detil_tes == "ORDER BY RAND() LIMIT 1";

               $whr_cek_sdh_ujian = ['id_ujian' => $id_ujian, 'id_peserta' => $data['sess_id']];
               $cek_sdh_ujian = $this->m_user->cek_sdh_ujian($whr_cek_sdh_ujian);

               if ($cek_sdh_ujian->num_rows() < 1) {
                    # code...
                    $soal_urut_ok = array();
                    //$whr_q_soal = ['kegiatan' => $cek_detil_tes->id_kegiatan];
                    // $kelas = $this->db->query('SELECT kelas FROM peserta WHERE id_peserta='.$this->session->id)->row();

                    $q_soal = $this->m_user->q_soal($acakan . $cek_detil_tes->id_kegiatan)->result();
                    $i = 0;
                    foreach ($q_soal as $q) {
                         # code...
                    ...
                    $q_ambil_soal = $this->db->query('SELECT * FROM ikut_ujian WHERE id_ujian = ' . $id_ujian . ' AND id_peserta = ' . $data['sess_id'] . '')->row();
                    ...
                    //view
                    $data['title'] = $data['detil_soal']->kegiatan . ' | ' . $data['detil_soal']->nama_ujian;
                    $data['judul'] = $data['detil_soal']->kegiatan;
                    $data['jdwlujian'] = $this->m_user->jadwal_ujian($this->session->id)->result();
                    // $whr_cek_jawab_berdasarkan_id = ['id_peserta' => $data['sess_id']];
                    $data['user'] = $this->m_user->jwb($this->session->id)->result();
               ...
               $xx = $this->m_user->cek_selesai_ujian(['id_peserta' => $data['sess_id'], 'id_ujian' => $id_ujian, 'status' => 'N'])->row();
     ...
     function ujian_simpan($id_tes)
     {
          ...
          // $update_ = substr($update_, 0);

          $bake = $this->db->query("UPDATE ikut_ujian SET list_jawaban='" . $update_ . "' WHERE id_tes='$id_tes' AND id_peserta='$id_peserta'");
     ...
     function ujian_akhir($id_tes)
     {
          ...
          // $hsalah = -1;
          // $hbenar = 4;
          // $tsalah = $jumlah_soal - $jumlah_benar;
          // $thitungsalah = $tsalah * $hsalah;
          // $thitungbenar = $jumlah_benar * $hbenar;
          // $nilai = $thitungbenar + $thitungsalah + 1;
          $this->db->query("UPDATE ikut_ujian SET jml_benar=" . $jumlah_benar . ", nilai='" . $nilai . "', list_jawaban='" . $update_ . "', status='N' WHERE id_tes='$id_tes' AND id_peserta='$id_peserta'");
          $a['status'] = 'ok';
          $this->j($a);
          exit;
     }
     ...
     function gantipass()
     {
          ...
          $whr = ['id_peserta' => $this->session->id];
     ...
     public function notjawab()
     {
          ...
          $whrjurusan = ['jurusan.nama_jurusan' => $this->session->userdata('jurusan')];
          $data['jdwlujian'] = $this->m_user->jadwal_ujian($this->session->id)->result();
          $data['list_jawaban'] = $this->m_user->nilai()->result();
